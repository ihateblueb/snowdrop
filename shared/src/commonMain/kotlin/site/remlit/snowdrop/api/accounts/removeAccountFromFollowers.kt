...
suspend fun removeAccountFromFollowers(id: String): ApiResponse<Relationship> = safeApiRequest {
	val accountId = getCurrentAccountId()
	val host = getCurrentAccountHost()
	val token = settings.getString("account_${accountId}_token", "")

	val req = httpClient.post("https://$host/api/v1/accounts/$id/remove_from_followers") {
