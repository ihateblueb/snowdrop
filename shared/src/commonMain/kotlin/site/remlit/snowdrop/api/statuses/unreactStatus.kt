...
suspend fun unreactStatus(
	...
	val req = if (getFeature("reactions_pleroma")) {
		httpClient.delete("https://$host/api/v1/pleroma/statuses/$id/reactions/$reaction") {
			header("Authorization", "Bearer $token")
		}
	} else {
		httpClient.post("https://$host/api/v1/statuses/$id/unreact/$reaction") {
			header("Authorization", "Bearer $token")
		}
	}
