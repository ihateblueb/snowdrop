...
suspend fun unreactStatus(id: String, reaction: String): ApiResponse<Status> = safeApiRequest {
	val accountId = getCurrentAccountId()
	val host = getCurrentAccountHost()
	val token = settings.getString("account_${accountId}_token", "")

	val req = httpClient.post("https://$host/api/v1/statuses/$id/unreact/$reaction") {
